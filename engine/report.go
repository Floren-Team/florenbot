...
func CreateReport(user_id uint64, text string) error {
	_, err := DB.Exec("INSERT INTO reports (user_id, text) VALUES (?, ?)", user_id, text)
	if err != nil {
		log.Println(err)
	}
	return err
...
func GetReports() []structs.Report {
	...
	rows, err := DB.Query("SELECT user_id, text FROM reports")
	if err != nil {
		log.Println(err)
	}
	for rows.Next() {
...
func HasReport(user_id uint64) (bool, error) {
	...
	err := DB.QueryRow(query, user_id).Scan(&exists)
	if err != nil {
		return false, err
	}
...
func DeleteReport(user_id uint64) error {
	_, err := DB.Exec("DELETE FROM reports WHERE user_id = ?", user_id)
	if err != nil {
		return err
	}
